...
7. Candidate — только proposal: никакие commands, skills, agents, rules или workflow docs не меняй автоматически. Без regression evidence verdict proposal — `inconclusive` и `do not change process`. При `quality improvement demonstrated` создаётся отдельная process-change feature/branch/worktree и human-reviewed PR/MR.
