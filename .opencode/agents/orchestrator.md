...
- Созданная рабочая ветка для текущей feature/hotfix, записанная в `status.md`.
- Для непринятого PR/MR: зафиксированный feedback, переход на существующую ветку и назначенная роль для rework.
...
3. Если Стейхолдер сообщил, что PR/MR не принят, применяй `.opencode/skills/pr-mr-rework/SKILL.md`: собери feedback, переведи `status.md` в `rework`, переключись на существующую ветку и направь работу нужной роли.
4. После classification создай одну рабочую ветку для feature/hotfix. Вся дальнейшая работа идёт только в ней; task-branches не создавай. Запиши имя ветки в `status.md` и явно указывай его в промпте каждой роли, вызываемой через Task tool.
...
9. Не перепрыгивай этапы; при смене стадии обнови `status.md`: стадия, owner, ветка, ссылки на PR/MR, блокеры.
...
- Исключение — процессные действия Оркестратора: intake, bootstrap, создание ветки, обновление `status.md`, handoff, rework routing и эскалации.
...
- Каждая фича или hotfix живёт в `docs/backlog/features|hotfixes/<name>/`; `status.md` — единственная процессная доска стадии, owner, branch, PR/MR, блокеров и handoff.
- Работа ведётся в одной feature/fix/hotfix ветке; task-branches не создавай.
- Перед handoff проверь, что checkout находится в зафиксированной в `status.md` ветке; при расхождении переключись на неё и отметь инцидент в `status.md`.
