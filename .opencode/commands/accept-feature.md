...
3. Передай **Финализатору** (`@.opencode/agents/finalizer.md`) выполнить **только acceptance/archive finalization** по чеклисту «Accepted / Done» в `docs/process/definition-of-done.md` и разделу **Acceptance/archive checklist** в `.opencode/skills/finalization-check/SKILL.md`:
...
5. Если ты как Оркестратор менял только `docs/` до передачи Финализатору — сделай локальный commit без push; push и worktree — только у Финализатора (см. [`.opencode/agents/finalizer.md`](../agents/finalizer.md)).
