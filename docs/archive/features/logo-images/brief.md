...
- Логотипы в результатах поиска (`/v1.5/movie/search`) — снято на analysis: проверено, что эндпоинт поиска не имеет `selectFields` и отдаёт `logo` в дефолтной схеме ответа (`SearchMovieDtoV1_4`). Вопрос о маппинге логотипа в fallback-ветке поиска вынесен в `analysis.md` (OQ-1).
